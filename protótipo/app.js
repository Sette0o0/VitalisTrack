...
function updateWater(amount) {
	...
	const liters = (waterTotal / 1000).toLocaleString("pt-BR", {
		maximumFractionDigits: 2,
	});
	document.querySelector("#water-total").textContent =
		`${waterTotal.toLocaleString("pt-BR")} mL`;
...
document.addEventListener("click", (event) => {
	...
	if (dialogButton)
		document.querySelector(`#${dialogButton.dataset.dialogOpen}`).showModal();
	if (event.target.closest("[data-sheet-open]")) openSheet();
	if (event.target.closest("[data-overlay-close]") || event.target === scrim)
		closeOverlays();
	...
	const tab = event.target.closest(
		".period-tabs button, .filter-chips button, .calendar-strip button, .segmented-buttons button",
	);
	if (tab) {
		...
		tab.classList.add(
			tab.parentElement.classList.contains("calendar-strip") ||
				tab.parentElement.classList.contains("segmented-buttons")
				? "selected"
				: "active",
		);
	}
...
document
	.querySelector("#custom-water-form")
...
function setTheme(isDark) {
	...
	).textContent = isDark ? "light_mode" : "dark_mode";
